...
float sun_x_position = 110.0f; // Posisi X matahari
...
bool isDraggingSun = false;
float sun_radius = 15.0f; // Radius matahari, digunakan untuk deteksi klik
float mouse_offset_x; // Offset X mouse dari pusat matahari saat dragging
float mouse_offset_y; // Offset Y mouse dari pusat matahari saat dragging
...
void drawSun()
{
    ...
    drawCircle(sun_x_position, sun_y_position, sun_radius, 1.0f, 0.9f, 0.2f);
}
...
void update(int value)
{
    if (animateDayNight) // Hanya jika animasi siang-malam diaktifkan
    {
        ...
        float lower_bound = -50.0f; // Batas bawah posisi Y matahari untuk efek siang/malam
        float upper_bound = 80.0f;  // Batas atas posisi Y matahari untuk efek siang/malam

        // Gerakkan matahari secara mulus menuju target Y
...
// Fungsi untuk mengonversi koordinat layar (pixel) ke koordinat OpenGL (world)
void screenToWorld(int screenX, int screenY, float *worldX, float *worldY) {
    int width = glutGet(GLUT_WINDOW_WIDTH);
    int height = glutGet(GLUT_WINDOW_HEIGHT);

    // Dapatkan batas koordinat OpenGL dari gluOrtho2D
    ...
    gluUnProject(screenX, height - screenY, 0.0, modelview, projection, viewport, &objX, &objY, &objZ);
...
// Fungsi callback mouse saat tombol ditekan atau dilepas
void mouse(int button, int state, int x, int y)
{
    if (button == GLUT_LEFT_BUTTON) // Jika tombol kiri mouse ditekan
    {
        float worldX, worldY;
        screenToWorld(x, y, &worldX, &worldY); // Konversi koordinat mouse ke koordinat dunia

        if (state == GLUT_DOWN) // Jika tombol ditekan
        {
            // Periksa apakah klik berada di dalam area matahari
            float dist_x = worldX - sun_x_position;
            float dist_y = worldY - sun_y_position;
            if (sqrt(dist_x * dist_x + dist_y * dist_y) < sun_radius)
            {
                isDraggingSun = true; // Set flag dragging ke true
                // Hitung offset mouse dari pusat matahari
                mouse_offset_x = worldX - sun_x_position;
                mouse_offset_y = worldY - sun_y_position;
                animateDayNight = false; // Hentikan animasi siang/malam saat digeser manual
            }
        }
        else if (state == GLUT_UP) // Jika tombol dilepas
        {
            isDraggingSun = false; // Set flag dragging ke false
        }
    }
}

// Fungsi callback mouse saat mouse digerakkan (dragging)
void mouseMotion(int x, int y)
{
    if (isDraggingSun) // Jika sedang dalam mode dragging matahari
    {
        float worldX, worldY;
        screenToWorld(x, y, &worldX, &worldY); // Konversi koordinat mouse ke koordinat dunia

        // Atur posisi matahari berdasarkan posisi mouse dan offset
        sun_x_position = worldX - mouse_offset_x;
        sun_y_position = worldY - mouse_offset_y;

        // Batasi pergerakan matahari agar tetap di dalam jendela (opsional)
        if (sun_x_position < -140.0f) sun_x_position = -140.0f;
        if (sun_x_position > 140.0f) sun_x_position = 140.0f;
        if (sun_y_position < -90.0f) sun_y_position = -90.0f;
        if (sun_y_position > 90.0f) sun_y_position = 90.0f;

        glutPostRedisplay(); // Minta penggambaran ulang
    }
}
...
void init()
{
    ...
    sun_x_position = 110.0f; // Memulai dengan posisi X matahari default
    sun_y_position = 80.0f; // Memulai dengan posisi Y matahari di atas (siang)
}
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("Scene Alam dengan Interaksi Mouse dan Siklus Siang-Malam"); // Membuat jendela
    ...
    glutMouseFunc(mouse); // Mendaftarkan fungsi callback mouse klik
    glutMotionFunc(mouseMotion); // Mendaftarkan fungsi callback mouse saat digerakkan (dragging)
